mppm.quad.ppp = function (formula, data, interaction = Poisson(), average_number = NULL, ..., iformula = NULL, 
                     random = NULL, weights = NULL, use.gam = FALSE, reltol.pql = 0.001, 
                     gcontrol = list(), run_quad = TRUE) 
{
  cl <- match.call()
  ...
  for (i in start:npat) {
    ...
    # }
    prep0 <- bt.frame.ppp(Yi, trend, Poisson(), ..., covariates = NULL, 
    ...
    for (j in (1:ninteract)[iused & !trivial]) {
      ...
      print(c('tomogram', i))
      prepj <- bt.frame.ppp(Yi, ~1, inter, ..., covariates = NULL, 
      ...
      glmdat$pattern_ID = rep(i, dim(glmdat)[1])
    }
    if (i == 1) {
      moadf <- glmdat
      # save(glmdat, file=paste("/data2/CancerECT/dataframe/Quad_", i, ".Rda", sep = ""))
      # rm(glmdat)
    }
    else {
      ...
      # }
      moadf = rbindlist(list(moadf, glmdat), fill = T)
    }
  }
 
  moadf = as.data.frame(moadf)
  moadf$caseweight <- 1
  if (backdoor) 
    return(moadf)
  ...
  return(list(moadf = moadf, fmla = fmla, Call = list(callstring = callstring, cl = cl), 
       Info = list(has.random = has.random, has.covar = has.covar, 
                   ...
                   rbord = prep0$info$rbord), Vnamelist = Vnamelist, Isoffsetlist = Isoffsetlist, 
       ...
       random = random, npat = npat, data = data, Y = Y, 
       datadf = datadf))
  }
